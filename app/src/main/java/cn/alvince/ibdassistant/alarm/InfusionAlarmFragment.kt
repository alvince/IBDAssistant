...
@Composable
fun AlarmScreen(modifier: Modifier = Modifier, viewModel: InfusionAlarmViewModel = viewModel()) {
    Surface(modifier.fillMaxSize()) {
        ConstraintLayout(modifier.fillMaxSize()) {
            ...
            ClockTime(modifier, viewModel.time, timeText, if (startAlarm) countDownTimeText else middleSpace)
            if (startAlarm) {
                CountDownTime(modifier, viewModel.countDownTime, countDownTimeText, timeText, middleSpace)
            }
            Spacer(
                modifier
                    .height(128.dp)
            ...
            if (startAlarm) {
                ...
                InfusionSpeed(modifier, viewModel.speed, speedText, speedSpace)
            }
            if (startAlarm) {
...
fun AlarmScreenPreview() {
    AlarmScreen(viewModel = InfusionAlarmViewModel())
}
